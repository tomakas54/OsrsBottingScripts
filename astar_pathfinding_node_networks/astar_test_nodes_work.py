...
def world_graph_nodes_names_paths(df):
    ...
    path = r'astar_pathfinding_node_networks/osrs_map_merged.png'
    # store image file to variable
    img = cv2.imread(path, 1)
    if img is None:
        raise ValueError(f"Image at path {path} could not be loaded. Check the path and file.")
    # loop through each coordinate pair in arr
    item = 0
    while item < len(df):
        # using cv2 draw a circle for the node points
        cv2.circle(img, (df[item][0], df[item][1]), radius=2, color=(0, 0, 255), thickness=10)
        ...
        if item == 0:
            ...
            pre_item = item - 1
        cv2.line(img, (df[item][0], df[item][1]), (df[pre_item][0], df[pre_item][1]), (0, 255, 0),10)
    ...
    output_path = 'TARGET--RES_WALKER_NAMES_PATHS_AUG_2021.png'
...
def screen_Image(left=0, top=0, name='rs_walker_x.png'):
        png = 'astar_pathfinding_node_networks/osrs_map_merged.png'
        ...
        im.save(r'astar_pathfinding_node_networks/rs_walker_image_path/' + name) # saves new cropped image
...
p1 = node_settings.locFaladorWBank
p2 = node_settings.locFaladorEBank
...
start = str(df_Nodes.index([p1[0]+3585,p1[1]-49]))
end = str(df_Nodes.index([p2[0]+3585,p2[1]-49]))
